...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Edit Template Undangan</h1>
    </div>
        ...
        <form action="{{ route('admin.templates.update', $template->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="3"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $template->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                @if($template->thumbnail)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $template->thumbnail) }}" alt="Current thumbnail" class="h-32 w-32 object-cover rounded border">
            ...
            <div class="mb-4">
                ...
                <textarea id="html_content" name="html_content" rows="15" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm @error('html_content') border-red-500 @enderror">{{ old('html_content', $template->html_content) }}</textarea>
                @error('html_content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
